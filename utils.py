...
def request_paper_with_arXiv_api(keyword: str, max_results: int, link: str = "OR") -> List[Dict[str, str]]:
    ...
    keyword = "\"" + keyword + "\""
    url = "http://export.arxiv.org/api/query?search_query=(cat:cs.SE+OR+cat:cs.AI+OR+cat:cs.IR+OR+cat:cs.LG)+AND+(ti:{0}+{2}+abs:{0})&max_results={1}&sortBy=lastUpdatedDate".format(keyword, max_results, link)
...
def filter_tags(papers: List[Dict[str, str]], target_fileds: List[str]=["cs.SE"]) -> List[Dict[str, str]]:
    # filtering tags: only keep the papers in target_fileds
    results = []
    for paper in papers:
        tags = paper.Tags
        for tag in tags:
            if tag in target_fileds:
